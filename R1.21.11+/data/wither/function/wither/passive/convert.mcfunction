...
execute at @e[type=wither,tag=ominousWither,limit=1,sort=nearest] as @e[type=#minecraft:zombies,distance=..20,tag=] unless data entity @s CustomName run tag @s add zMarked

execute at @e[type=wither,tag=ominousWither,limit=1,sort=nearest] as @e[nbt={Tags:["zMarked"]}] at @s run summon zombified_piglin ~ ~.01 ~ {Team:"Wither",DeathLootTable: "wither:entities/withermobs", Tags: ["wSkel"], attributes: [{id: "max_health", base: 10}], anger_end_time:999999, Health: 10}

execute at @e[type=wither,tag=ominousWither,limit=1,sort=nearest] as @e[nbt={Tags:["zMarked"]}] run data merge entity @s {Silent:1b,DeathLootTable:"wither:entities/withermobs",equipment:{head:{id:"minecraft:bone",count:1}},active_effects:[{id:"minecraft:invisibility",amplifier:1,duration:20,show_particles:0b}],attributes:[{id:"minecraft:scale",base:0.0625}]}

execute at @e[type=wither,tag=ominousWither,limit=1,sort=nearest] as @e[nbt={Tags:["zMarked"]}] run kill @s
...
execute at @e[type=wither,tag=ominousWither,limit=1,sort=nearest] as @e[nbt={Tags:["sMarked"]}] run data merge entity @s {Silent:1b,DeathLootTable:"wither:entities/withermobs",equipment:{head:{id:"minecraft:bone",count:1}},active_effects:[{id:"minecraft:invisibility",amplifier:1,duration:20,show_particles:0b}],attributes:[{id:"minecraft:scale",base:0.0625}]}
