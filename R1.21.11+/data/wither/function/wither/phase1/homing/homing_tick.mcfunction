...
# Homing Explosion
execute as @e[type=minecraft:armor_stand,limit=1,sort=nearest,tag=hStand] at @s if entity @e[type=player,distance=..0.45] run summon creeper ~ ~ ~ {Team:"Wither",Tags: ["hboom"], CustomName:"Ominous Wither's Homing Skull", Fuse: 0, ExplosionRadius: 2, Silent: 1b, attributes:[{id:"minecraft:scale",base:0.0625}]}
...
execute as @e[type=minecraft:armor_stand,limit=1,sort=nearest,tag=hStand] at @s anchored eyes positioned ^ ^ ^ unless block ^ ^ ^1 air run summon creeper ~ ~ ~ {Team:"Wither",Tags: ["hboom"], CustomName:"Ominous Wither's Homing Skull", Fuse: 0, ExplosionRadius: 2, Silent: 1b, attributes:[{id:"minecraft:scale",base:0.0625}]}
...
execute if entity @e[type=minecraft:armor_stand,tag=hStand] run schedule function wither:wither/phase1/homing/homing_tick 1t
