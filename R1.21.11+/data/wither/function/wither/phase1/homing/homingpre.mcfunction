...
execute if entity @e[type=wither,tag=ominousWither,scores={Health=..599}] if entity @e[limit=1,sort=nearest,tag=hStand,type=minecraft:armor_stand] if data storage wither:options {togglehoming:Enabled} run schedule function wither:wither/phase1/homing/homingtick 1t
